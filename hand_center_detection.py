...
#czas kalibracji, kolor skóry i tak dalej
CALIBRATION_TIME = 30
BG_WEIGHT = 0.5
OBJ_THRESHOLD = 18


#klasa dloni zawieracjaca najwazniejsze informacji, niezbedne dla tej metody
class HandData:
    top = (0,0)
    bottom = (0,0)
    left = (0,0)
...
def get_region(frame):
    #sepracja ROI od reszty okna
    region = frame[region_top:region_bottom, region_left:region_right]
    #ukazujemy ROI w sklai szarosci
...
def get_average(region):
    ...
    if background is None:
        ...
        return
    #dodanie tla do regionu
    cv2.accumulateWeighted(region, background, BG_WEIGHT)
    
# seperacja tla od naszej dloni
def segment(region):
    global hand
    # roznica bewzgledna, a potem wrzucamy to do funkcji threshold
    diff = cv2.absdiff(background.astype(np.uint8), region)

    #nasza dlon zostaje na pierwszym planie
    thresholded_region = cv2.threshold(diff, OBJ_THRESHOLD, 255, cv2.THRESH_BINARY)[1]

    # znajdujemy kontur naszej dloni
    (contours, _) = cv2.findContours(thresholded_region.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # Brak konturu = brak dloni
    if len(contours) == 0:
        if hand is not None:
            hand.isInFrame = False
        return
    # jesli reka wykryta zwracamy wysegmentowana dlon
    else:
        if hand is not None:
            hand.isInFrame = True
...
def get_hand_data(thresholded_image, segmented_image):
    ...
    # Szukanie punktow ekstremalnych
    top    = tuple(convexHull[convexHull[:, :, 1].argmin()][0])
    bottom = tuple(convexHull[convexHull[:, :, 1].argmax()][0])
    left   = tuple(convexHull[convexHull[:, :, 0].argmin()][0])
    right  = tuple(convexHull[convexHull[:, :, 0].argmax()][0])
    
    # okreslenie centrum dloni
...
def count_fingers(thresholded_image):
    ...
    # linia na wysokosci palcow
    line = np.zeros(thresholded_image.shape[:2], dtype=int)
    
    cv2.line(line, (thresholded_image.shape[1], line_height), (0, line_height), (255, 0,0))
    
    # liczymy ilosc przeciec dloni
...
while (True):
    ...
    if frames_elapsed < CALIBRATION_TIME:
        ...
        if region_pair is not None:
            ...
            hand.fingers = count_fingers(thresholded_region)
